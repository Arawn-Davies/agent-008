#!/usr/bin/env python3
"""UserPromptSubmit hook: surface new agent_im messages as context, silently.

Never errors and never blocks the prompt — any failure just means no
context gets added. Run standalone: `python3 agent_im_hook.py`.
"""
import json
import os
import shutil
...
def find_agent_im():
    found = shutil.which("agent_im")
    if found:
        return found
    fallback = os.path.expanduser("~/agent-008/bin/agent_im")
    return fallback if os.access(fallback, os.X_OK) else None


def main():
    agent_im = find_agent_im()
    if not agent_im:
        return

    config_path = os.path.expanduser("~/.agent_im/config.json")
    ...
    context = (
        f"You have {len(messages)} new agent_im message(s) from your collaborator "
        ...
        "pull it out with `agent_im recv --save-patches <dir>` before applying."
    )
    print(json.dumps({
        "hookSpecificOutput": {
